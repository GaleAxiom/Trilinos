...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void ShiftedLaplacian<Scalar, LocalOrdinal, GlobalOrdinal, Node>::setParametersSolver(Teuchos::RCP<Teuchos::ParameterList> paramList) {
    // Create Belos parameter list if it doesn't exist
    if (BelosList_ == Teuchos::null) {
    ...
    if (verbosityLevel == 0) {
        ...
        verbosity = Belos::Errors + Belos::Warnings + Belos::StatusTestDetails + Belos::TimingDetails;
    }
    
    // Output style
    std::string outputStyle = paramList->get("Solver: output style", "Brief");
    auto style = 0;
    if (outputStyle == "None") {
        style = Belos::Brief;
    } else if (outputStyle == "General") {
        style = Belos::General;
    } else if (outputStyle == "Brief") {
        style = Belos::Brief;
    }
    
    // Advanced parameters (optional)
    int outputFrequency = paramList->get("Solver: output frequency", 1);
    bool explicitResidual = paramList->get("Solver: explicit residual", false);
    bool adaptiveBlockSize = paramList->get("Solver: adaptive block size", false);
    bool useIROrthoManager = paramList->get("Solver: use IR orthogonalization", false);
    
    // Update the Belos parameter list
    BelosList_->set("Maximum Iterations", iters_);
    BelosList_->set("Convergence Tolerance", tol_);
    BelosList_->set("Verbosity", verbosity);
    BelosList_->set("Output Frequency", outputFrequency);
    BelosList_->set("Output Style", style);
    BelosList_->set("Num Blocks", restart_size_);
    BelosList_->set("Num Recycled Blocks", recycle_size_);
    
    // Optional advanced parameters
    if (paramList->isParameter("Solver: explicit residual")) {
        BelosList_->set("Explicit Residual Test", explicitResidual);
    }
    if (paramList->isParameter("Solver: adaptive block size")) {
        BelosList_->set("Adaptive Block Size", adaptiveBlockSize);
    }
    if (paramList->isParameter("Solver: use IR orthogonalization")) {
        BelosList_->set("Use Implicit Restarted Arnoldi", useIROrthoManager);
    }
    
    // If the solver has already been created, we need to reset it with the new parameters
    if (SolverManager_ != Teuchos::null) {
        std::string solverName;
        if (solverType_ == 1) {
            solverName = "Block GMRES";
...
// Solve phase
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void ShiftedLaplacian<Scalar, LocalOrdinal, GlobalOrdinal, Node>::multigrid_apply(
    const RCP<MultiVector> B, RCP<MultiVector>& X) {
    // Set left and right hand sides for Belos
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void ShiftedLaplacian<Scalar, LocalOrdinal, GlobalOrdinal, Node>::multigrid_apply(
    const RCP<Tpetra::MultiVector<SC, LO, GO, NO>> B, RCP<Tpetra::MultiVector<SC, LO, GO, NO>>& X) {
